...
public class BluetoothP2pPlugin: NSObject, FlutterPlugin, CBCentralManagerDelegate, CBPeripheralDelegate {
  ...
  private func getBatteryPercentage(result: @escaping FlutterResult) {
    // Get battery info using IOKit for macOS
    ...
    for powerSource in powerSources {
      let powerSourceDict = IOPSGetPowerSourceDescription(powerSourceInfo, powerSource)?.takeUnretainedValue()
      
      if let dict = powerSourceDict as? [String: Any],
    ...
    // If no battery found (desktop Mac), return error
    result(FlutterError(code: "BATTERY_ERROR", message: "No battery found on this device", details: nil))
  }
  ...
  private func getDeviceType(peripheral: CBPeripheral) -> Int {
    // macOS doesn't provide classic Bluetooth type info, assume BLE
    return 2 // LE type
  }
  
  private func getBondState(peripheral: CBPeripheral) -> Int {
    // macOS doesn't expose bond state directly, return unpaired
